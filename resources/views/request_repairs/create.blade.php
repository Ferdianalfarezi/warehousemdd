    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col">
        ...
        <div class="overflow-y-auto flex-1 p-6">
            <form id="createForm" class="space-y-5">
                ...
                {{-- Diajukan Oleh --}}
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">Diajukan Oleh</label>
                    @if(auth()->user()->role_id == 1)
                        <select name="created_by" id="createCreatedBy"
                            class="w-full border border-gray-300 rounded-lg focus:ring-2 focus:ring-black">
                            <option value="">Default: Diri Sendiri</option>
                        </select>
                        <p class="text-xs text-gray-400 mt-1">Kosongkan untuk atas nama sendiri, atau cari & pilih user.</p>
                    @else
                        <input type="text" value="{{ auth()->user()->nama }}" disabled readonly
                            class="w-full px-3 py-2 border border-gray-200 bg-gray-50 rounded-lg text-gray-600 cursor-not-allowed">
                    @endif
                    <p class="error-message text-xs text-red-500 mt-1" id="error-create-created_by"></p>
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-start space-x-2">
                        ...
                        <button type="button" onclick="openQuickAddBarangModal()" title="Tambah Part Baru"
                            class="flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-lg bg-black text-white hover:bg-gray-800 transition">
